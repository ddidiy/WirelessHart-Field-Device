...
#include "C818_WriteChannelBlacklist.h"

uint8_t Compose_C818_WriteChannelBlacklist_Req ( C818_WriteChannelBlacklist_Req* request,
                                    BinaryStream* toStream )
{
  STREAM_WRITE_UINT8(toStream, request->m_ucNoOfBitsInNewChannelMapArray);
  STREAM_WRITE_UINT16(toStream, request->m_unPendingChannelMapArray);
...
{
#if defined(_DEBUG)
  if (STREAM_GetRemainingSize(fromStream) > C818_ReqSize)
    return RCS_E01_Undefined1; // too many bytes
#endif

  STREAM_READ_UINT8(fromStream, &request->m_ucNoOfBitsInNewChannelMapArray);
  STREAM_READ_UINT16(fromStream, &request->m_unPendingChannelMapArray);
...
uint8_t Compose_C818_WriteChannelBlacklist_Resp ( C818_WriteChannelBlacklist_Resp* response,
                                    BinaryStream* toStream )
{
  STREAM_WRITE_UINT8(toStream, response->m_ucNoOfBitsInNewChannelMapArray);
...
{
#if defined(_DEBUG)
  if (STREAM_GetRemainingSize(fromStream) > C818_RespSize)
    return RCS_E01_Undefined1; // too many bytes
#endif

  STREAM_READ_UINT8(fromStream, &response->m_ucNoOfBitsInNewChannelMapArray);
