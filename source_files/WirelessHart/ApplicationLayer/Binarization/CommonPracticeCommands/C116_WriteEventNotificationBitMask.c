		 ...
		 BinaryStream* fromStream)
{
	...
	request->noOfWriteEvents = STREAM_GetRemainingSize(fromStream);
        if(MaxNoOfWriteEvents < request->noOfWriteEvents)
          request->noOfWriteEvents = MaxNoOfWriteEvents;
	for (uint8_t i = 0; i < request->noOfWriteEvents; i++)
	{
		 ...
		 BinaryStream* fromStream)
{
	...
	response->noOfWriteEvents = STREAM_GetRemainingSize(fromStream);
	for (uint8_t i = 0; i < response->noOfWriteEvents; i++)
	{
