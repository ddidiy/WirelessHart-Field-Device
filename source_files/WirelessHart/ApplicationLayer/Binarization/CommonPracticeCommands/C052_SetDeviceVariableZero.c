...
#include "C052_SetDeviceVariableZero.h"

uint8_t Compose_C052_SetDeviceVariableZero_Req(C052_SetDeviceVariableZero_Req* request, 
		BinaryStream* toStream)
{
	STREAM_WRITE_UINT8(toStream, request->variableToBeZeroed);
...
{
	STREAM_READ_UINT8(fromStream, &request->variableToBeZeroed);
	return RCS_N00_Success;
}
...
uint8_t Compose_C052_SetDeviceVariableZero_Resp(C052_SetDeviceVariableZero_Resp* response, 
		BinaryStream* toStream)
{
	STREAM_WRITE_UINT8(toStream, response->variableToBeZeroed);
...
{
	STREAM_READ_UINT8(fromStream, &response->variableToBeZeroed);
	return RCS_N00_Success;
}
